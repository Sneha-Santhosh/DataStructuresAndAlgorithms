...
//The following inorderNext method accepts a position p and then returns a
//position visited after the position p in a inorder traversal of a binary
//tree T.
//In the inorder traversal, traverse the left subtree, get the current
//position, and traverse the right subtree.
...
//The following postorderNext method accepts a position p and then returns
//a position visited after the position p in a postorder traversal of a
//binary tree T.
//In the postorder traversal, traverse the left subtree, traverse the right
//subtree, and get the current position.
public Position<E> postorderNext(Position<E> p)
{
   ...
   while(k != null && (left(k) != null || right(k) != null))
   {
        if(left(k) != null)
            k = left(k);
        else
